...
import { getPendingUsers, getAllMembers, PendingUser, approveUser, rejectUser } from '@/services/members';
...
export default function Members() {
  ...
  useEffect(() => {
    const fetchAllMembers = async () => {
      ...
      try {
        ...
        const errorMessage = err instanceof Error ? err.message : "Failed to fetch all members";
        toast({
          variant: "destructive",
          title: "Error",
          description: errorMessage,
  ...
  const handleApprove = async (userId: string, username: string) => {
    ...
    try {
      ...
      setAllMembers(prev => prev.map(user => 
      ...
      <div className="p-3 sm:p-4 md:p-8">
        ...
        <div className="mb-6 sm:mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              ...
              <p className="text-sm sm:text-base text-muted-foreground">Manage and review all members</p>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="bg-card border border-border rounded-lg p-6">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm text-muted-foreground mb-1">Active Pending</p>
                <p className="text-2xl font-bold text-foreground">{activePending}</p>
              </div>
            ...
            {isLoading ? (
              ...
              <>
                  ...
                  <table className="w-full min-w-[700px]">
                    <thead>
                      <tr className="border-b border-border bg-slate-800 bg-opacity-50">
                        <th className="text-left py-4 px-6 font-semibold text-foreground min-w-[200px]">Username</th>
                        <th className="text-left py-4 px-6 font-semibold text-foreground min-w-[250px] hidden sm:table-cell">Email</th>
                        <th className="text-left py-4 px-6 font-semibold text-foreground min-w-[120px]">Role</th>
                        <th className="text-left py-4 px-6 font-semibold text-foreground min-w-[140px] hidden md:table-cell">Applied Date</th>
                        <th className="text-left py-4 px-6 font-semibold text-foreground min-w-[120px]">Status</th>
                        <th className="text-center py-4 px-6 font-semibold text-foreground min-w-[120px]">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                ...
                {filteredPendingUsers.length === 0 && pendingUsers.length > 0 && (
                  <div className="text-center py-12">
                    <p className="text-muted-foreground text-lg">No pending users found matching your search</p>
                  </div>
                )}
                {pendingUsers.length === 0 && !isLoading && (
                  <div className="text-center py-12">
                    <p className="text-muted-foreground text-lg">No pending approvals</p>
                    <p className="text-sm text-muted-foreground mt-2">All member applications have been processed</p>
                  </div>
                )}
              </>
            )}
                  ...
                  <table className="w-full min-w-[700px]">
                    <thead>
                      <tr className="border-b border-border bg-slate-800 bg-opacity-50">
                        ...
                        <th className="text-left py-4 px-6 font-semibold text-foreground min-w-[250px] hidden sm:table-cell">Email</th>
                        <th className="text-left py-4 px-6 font-semibold text-foreground min-w-[120px]">Role</th>
                        <th className="text-left py-4 px-6 font-semibold text-foreground min-w-[140px] hidden md:table-cell">Joined Date</th>
                        <th className="text-left py-4 px-6 font-semibold text-foreground min-w-[120px]">Status</th>
                        <th className="text-left py-4 px-6 font-semibold text-foreground min-w-[140px] hidden md:table-cell">Last Login</th>
                      </tr>
                    </thead>
                    <tbody>
